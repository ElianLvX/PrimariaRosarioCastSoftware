 ...
 * @author ElianLv
 */
public class matcal_3 extends javax.swing.JFrame {
    ...
    private void guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarActionPerformed
        try {
            ...
            // Almacen de calificacion para despues convertir a float
            String conver = "";
            ...
            // Procesando para solucion
            //pps.setInt(1, periodo); // Aqui resivo el periodo actual
            //pps.setFloat(2, valorCalificacion); // Aqui resivo la calificacion
            //pps.setInt(3, valorAlumno); // aqui resivo el id del alumno
            //pps.setInt(4, valorMateria); // Aqui resivo el id de la materia
            pps.executeUpdate();
            limpiar();
    ...
    private void actualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_actualizarActionPerformed
        ...
        if(valorCaliIngles == 0){
            ...
            System.out.println("Valor Sin: " + conver);
            promedio = Float.parseFloat(conver);
            JOptionPane.showMessageDialog(null, "Promedio Alcanzado: "+ String.format("%.1f",promedio));
        }
        // Caso 4 si materia ConMedio no se Coloco
        if(valorCaliCieNaturales == 0){
            promedio = (valorCaliEspañol+valorCaliMatematicas+valorCaliIngles+valorCaliEntidadDonde+valorCaliFormCivica+valorCaliArtes+valorCaliEduFisica)/7;
        ...
        // Caso 5 si materia Entidad donde Vivo no se coloco
        if(valorCaliEntidadDonde == 0){
            promedio = (valorCaliEspañol+valorCaliMatematicas+valorCaliIngles+valorCaliCieNaturales+valorCaliFormCivica+valorCaliArtes+valorCaliEduFisica)/7;
            conver = String.valueOf(promedio);
            conver = conver.substring(0,3);
        ...
        try {
            pps = (PreparedStatement) cn.prepareStatement("UPDATE matcal_3 SET español='"+txtEspañol.getText()+"',matematicas='"
                ...
                "',artes='"+txtArtes.getText()+"',educacion_fisica='"+txtEducacionFisica.getText()+"',promedio='"+String.format("%.1f",promedio)+"' WHERE id='"+id_alumnocal.getText()+"';");
            pps.execute();
